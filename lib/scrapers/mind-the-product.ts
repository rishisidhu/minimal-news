...
export async function scrapeMindTheProduct(): Promise<NewsArticle[]> {
  try {
    const response = await axios.get('https://www.mindtheproduct.com/', {
      timeout: 10000,
      headers: {
    ...
    for (const selector of articleSelectors) {
      $(selector).each((i, elem) => {
        ...
        if (!url.startsWith('http')) {
          url = `https://www.mindtheproduct.com${url}`
        }
    ...
    console.log(`Mind the Product: Scraped ${articles.length} articles`)
    return articles
  } catch (error) {
    console.error('Error scraping Mind the Product:', error)
    return []
  }
}
