...
export async function scrapeProductPlan(): Promise<NewsArticle[]> {
  try {
    const response = await axios.get('https://www.productplan.com/blog/', {
      timeout: 10000,
      headers: {
    ...
    for (const selector of articleSelectors) {
      $(selector).each((i, elem) => {
        ...
        if (!url.startsWith('http')) {
          url = `https://www.productplan.com${url}`
        }

        if (seenUrls.has(url) || url === 'https://www.productplan.com/blog/') return
        seenUrls.add(url)
    ...
    console.log(`ProductPlan: Scraped ${articles.length} articles`)
    return articles
  } catch (error) {
    console.error('Error scraping ProductPlan:', error)
    return []
  }
}
