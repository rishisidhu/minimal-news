...
export async function scrapeOpenAI(): Promise<NewsArticle[]> {
  try {
    ...
    for (const selector of articleSelectors) {
      $(selector).each((i, elem) => {
        ...
        if (fullUrl.includes('/blog/') === false && fullUrl.includes('/index/') === false) return
        ...
        if (img) {
          imageUrl = img.startsWith('http') ? img : `https://openai.com${img}`
        }
    ...
    console.log(`OpenAI: Scraped ${articles.length} articles`)
    return articles
  } catch (error) {
