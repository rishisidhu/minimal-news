...
export async function scrapeProductTalk(): Promise<NewsArticle[]> {
  try {
    const response = await axios.get('https://www.producttalk.org/blog/', {
      timeout: 10000,
      headers: {
    ...
    for (const selector of articleSelectors) {
      $(selector).each((i, elem) => {
        ...
        if (!url.startsWith('http')) {
          url = `https://www.producttalk.org${url}`
        }

        if (seenUrls.has(url) || url === 'https://www.producttalk.org/blog/') return
        seenUrls.add(url)
    ...
    console.log(`ProductTalk: Scraped ${articles.length} articles`)
    return articles
  } catch (error) {
    console.error('Error scraping ProductTalk:', error)
    return []
  }
}
