...
export async function scrapeMetaAI(): Promise<NewsArticle[]> {
  try {
    ...
    for (const selector of articleSelectors) {
      $(selector).each((i, elem) => {
        ...
        if (!fullUrl.includes('/blog/') || fullUrl === 'https://ai.meta.com/blog/') return
        ...
        if (img) {
          imageUrl = img.startsWith('http') ? img : `https://ai.meta.com${img}`
        }
    ...
    console.log(`Meta AI: Scraped ${articles.length} articles`)
    return articles
  } catch (error) {
    console.error('Error scraping Meta AI:', error)
    return []
  }
}
