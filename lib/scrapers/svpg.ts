...
export async function scrapeSVPG(): Promise<NewsArticle[]> {
  try {
    const response = await axios.get('https://svpg.com/articles/', {
      timeout: 10000,
      headers: {
    ...
    for (const selector of articleSelectors) {
      $(selector).each((i, elem) => {
        ...
        if (!url.startsWith('http')) {
          url = `https://svpg.com${url}`
        }

        if (seenUrls.has(url) || url === 'https://svpg.com/articles/') return
        seenUrls.add(url)
    ...
    console.log(`SVPG: Scraped ${articles.length} articles`)
    return articles
  } catch (error) {
    console.error('Error scraping SVPG:', error)
    return []
  }
}
